...
def main() -> int:
    ...
    ap.add_argument(
        "--whisper",
        choices=["local-whisperx"],
        default=None,
        help="Backend selector. Only local-whisperx is supported in this fork; flag exists for upstream API compat.",
    )
    args = ap.parse_args()
    ...
    if not transcript_segments and not args.no_whisper:
        ...
        if backend:
            ...
            hint = (
                "--whisper was set but local WhisperX is unreachable"
                if args.whisper else
                "no subtitles and local WhisperX is unreachable"
            )
